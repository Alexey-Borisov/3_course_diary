# Дневник по научной работе у Дьяконова А.Г. (не активный раздел)
...
## 01.04.2022
  ...
  Основная идея статьи состоит в том, чтобы объединить стадии извлечения признаков и собственно metric learning в одну оптимизационную проблему. Обычно в методах metric learning стараются обучить матрицу M, чтобы соответствующая метрика наилучшим образом разделяла классы, но в данном случае плюсом к этому авторы обучают новые представления для объектов, используя подход из dictionary learning. Также при получении итоговой задачи минимизации строится лапласиан графа по входным данным. Для решения задачи используют условия Каруша-Куна-Таккера и итоговая оптимизация представляет собой поочередную оптимизацию 5 (!) матриц. Вопрос теоретической сходимости такого процесса остается открытым, но в статье вводят дополнительный параметр шага, отвечающий за величину обновления параметров и авторы утверждают, что при выборе подходящего значения для этого параметра достигается сходимость. Это подтверждают и представленные в статье эксперименты. Итоговый алгоритм получился более устойчивым к добавлению шума и более сильному изменению изображений, чем некоторые другие алгоритмы metric learning участвовавшие в сравнении, в частности LMNN.
  
  ___
  
  Приступил к написанию обзорной статьи. Хотелось бы представить первый вариант в течении 7-10 дней (8-11 апреля).
